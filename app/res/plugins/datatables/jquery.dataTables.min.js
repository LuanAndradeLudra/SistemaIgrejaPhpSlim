...
(function( $, window, document, undefined ) {
	...
	function _fnCompatOpts ( init )
	{
		...
		_fnCompatMap( init, 'Pesquisaring',     'bFilter' );
		...
		// element by element
		var PesquisarCols = init.aoPesquisarCols;
	
		if ( PesquisarCols ) {
			for ( var i=0, ien=PesquisarCols.length ; i<ien ; i++ ) {
				if ( PesquisarCols[i] ) {
					_fnCamelToHungarian( DataTable.models.oPesquisar, PesquisarCols[i] );
				}
			}
		}
	}
	...
	function _fnAddColumn( oSettings, nTh )
	{
		...
		// Add Pesquisar object for column specific Pesquisar. Note that the `PesquisarCols[ iCol ]`
		// passed into extend can be undefined. This allows the user to give a default
		// with only some of the parameters defined, and also not give a default
		var PesquisarCols = oSettings.aoPrePesquisarCols;
		PesquisarCols[ iCol ] = $.extend( {}, DataTable.models.oPesquisar, PesquisarCols[ iCol ] );
	...
	function _fnAjaxParameters( settings )
	{
		...
		for ( i=0 ; i<columnCount ; i++ ) {
			...
			d.columns.push( {
				data:       dataProp,
				name:       column.sName,
				Pesquisarable: column.bPesquisarable,
	...
	function _fnFeatureHtmlFilter ( settings )
	{
		...
		var PesquisarDelay = settings.PesquisarDelay !== null ?
			settings.PesquisarDelay :
		...
		var jqFilter = $('input', filter)
			...
			.on(
				...
				PesquisarDelay ?
					_fnThrottle( PesquisarFn, PesquisarDelay ) :
					PesquisarFn
			)
			.on( 'mouseup', function(e) {
				// Edge fix! Edge 17 does not trigger anything other than mouse events when clicking
				// on the clear icon (Edge bug 17584515). This is safe in other browsers as `PesquisarFn`
				// checks the value to see if it has changed. In other browsers it won't have.
				setTimeout( function () {
					PesquisarFn.call(jqFilter[0]);
				}, 10);
			} )
	 ...
	 *  @memberof DataTable#oApi
	 */
	function _fnFilterColumn ( settings, PesquisarStr, colIdx, regex, smart, caseInsensitive )
	{
		if ( PesquisarStr === '' ) {
			return;
		}
		...
		var rpPesquisar = _fnFilterCreatePesquisar( PesquisarStr, regex, smart, caseInsensitive );
	...
	/**
	 * Build a regular expression object suitable for Pesquisaring a table
	...
	function _fnSaveState ( settings )
	{
		...
		var state = {
			...
			Pesquisar:  _fnPesquisarToCamel( settings.oPreviousPesquisar ),
			columns: $.map( settings.aoColumns, function ( col, i ) {
				return {
					visible: col.bVisible,
					Pesquisar: _fnPesquisarToCamel( settings.aoPrePesquisarCols[i] )
				};
			} )
		};
	...
	function _fnLoadState ( settings, oInit, callback )
	{
		...
		var loaded = function ( s ) {
			...
			// Pesquisar
			if ( s.Pesquisar !== undefined ) {
				$.extend( settings.oPreviousPesquisar, _fnPesquisarToHung( s.Pesquisar ) );
			}
			...
			if ( s.columns ) {
				for ( i=0, ien=s.columns.length ; i<ien ; i++ ) {
					...
					if ( col.Pesquisar !== undefined ) {
						$.extend( settings.aoPrePesquisarCols[i], _fnPesquisarToHung( col.Pesquisar ) );
					}
				}
			}
	...
	var _selector_row_indexes = function ( settings, opts )
	{
		...
		if ( _fnDataSource( settings ) == 'ssp' ) {
			...
			else if ( Pesquisar == 'applied' ) {
				a = displayFiltered.slice();
			}
			else if ( Pesquisar == 'removed' ) {
	...
	DataTable.models.oColumn = {
		...
		/**
		 * Flag to indicate if the column is Pesquisarable, and thus should be included
	...
	DataTable.defaults = {
		 ...
		 *
		 *  @dtopt Option
		 *  @name DataTable.defaults.PesquisarCols
		 *
		 *  @example
		 *    $(document).ready( function() {
		 *      $('#example').dataTable( {
		 ...
		 *
		 *  @dtopt Features
		 *  @name DataTable.defaults.Pesquisaring
		 *
		 *  @example
		 *    $(document).ready( function () {
		 *      $('#example').dataTable( {
		...
		"oLanguage": {
			 ...
			 *    } );
			 */
			"sInfo": "<small>Mostrando registro do _START_ até o _END_ de um total de _TOTAL_ registros</small>",
			 ...
			 *    } );
			 */
			"sInfoEmpty": "Mostrnado 0 de 0 entradas encontradas",
			 ...
			 *    } );
			 */
			"sInfoFiltered": "<small>(Filtrando _MAX_ registros)</small>",
			 ...
			 *
			 *  @dtopt Language
			 *  @name DataTable.defaults.language.PesquisarPlaceholder
			 */
		...
		/**
		 * Pesquisar delay option. This will throttle full table Pesquisares that use the
		 ...
		 *
		 *  @dtopt Options
		 *  @name DataTable.defaults.PesquisarDelay
		 *
		 *  @example
		 *    $(document).ready( function() {
		 *      $('#example').dataTable( {
	...
	DataTable.defaults.column = {
		 ...
		 *  @default true
		 *
		 *  @name DataTable.defaults.column.Pesquisarable
		 *  @dtopt Columns
		 *
		 *  @example
		 *    // Using `columnDefs`
	...
	DataTable.ext = _ext = {
		 ...
		 * Row Pesquisaring.
		 * 
		 * This method of Pesquisaring is complimentary to the default type based
		 * Pesquisaring, and a lot more comprehensive as it allows you complete control
		 * over the Pesquisaring logic. Each element in this array is a function
		 * (parameters described below) that is called for every row in the table,
		 * and your logic decides if it should be included in the Pesquisaring data set
		 * or not.
		 *
		 ...
		 * And the following return is expected:
		 *
		 * * {boolean} Include the row in the Pesquisared result set (true) or not
		 *   (false)
		 *
		 * Note that as with the main Pesquisar ability in DataTables, technically this
		 * is "filtering", since it is subtractive. However, for consistency in
		 * naming we call it Pesquisaring here.
		 *
		 *  @type array
		 *  @default []
		 *
		...
		type: {
			 ...
			 *
			 * The type based Pesquisaring functions can be used to pre-format the
			 * data to be Pesquisar on. For example, it can be used to strip HTML
			 * tags or to de-format telephone numbers for numeric only Pesquisaring.
			 *
			 * Note that is a Pesquisar is not defined for a column of a given type,
			 * no Pesquisar formatting will be performed.
			 * 
			 * Pre-processing of Pesquisaring data plug-ins - When you assign the sType
			 * for a column (or have it automatically detected for you by DataTables
			 * or a type detection plug-in), you will typically be using this for
			 * custom sorting, but it can also be used to provide custom Pesquisaring
			 * by allowing you to pre-processing the data and returning the data in
			 * the format that should be Pesquisared upon. This is done by adding
			 * functions this object with a parameter name which matches the sType
			 * for that target column. This is the corollary of <i>afnSortData</i>
			 * for Pesquisaring data.
			 *
			 * The functions defined take a single parameter:
			 *
		     *  1. `{*}` Data from the column cell to be prepared for Pesquisaring
			 *
			 * Each function is expected to return:
			 *
			 * * `{string|null}` Formatted string that will be used for the Pesquisaring.
			 *
			 *  @type object
			 *  @default {}
			 *
	...
	/**
	 * Pesquisar event, fired when the Pesquisaring applied to the table (using the
	 ...
	 * (be it, order, Pesquisar or anything else). It can be used to indicate to
